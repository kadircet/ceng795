  ...
  if (material.mirror != zero_vector && current_recursion_depth > 0) {
    if (material.roughness == 0.0f) {
      ...
      glossy_generator_u.seed(
        std::chrono::system_clock::now().time_since_epoch().count());
      std::uniform_real_distribution<float> glossy_distribution_u(-0.5f, 0.5f);
      std::default_random_engine glossy_generator_v;
      glossy_generator_v.seed(
        std::chrono::system_clock::now().time_since_epoch().count());
      std::uniform_real_distribution<float> glossy_distribution_v(-0.5f, 0.5f);
  ...
  try
  {
    ...
    if (ply_vertices) {
      ...
      if (ply_vertices->t == tinyply::Type::FLOAT32) {
        ...
        for (; index < c; index++) {
          output_vertices.push_back(Vector3(verts[3 * index], verts[3 * index + 1], verts[3 * index + 2]));
        }
      }
        ...
        if (is_triangle) {
          for (; index < c; index++) {
            ...
            if (!has_vertice_normals) {
              ...
              output_vertices[triangle->index_2].add_vertex_normal(surface_normal, area);
            }
          }
          ...
          for (; index < c; index++) {
            ...
            if (!has_vertice_normals) {
              ...
              }
              {
                ...
                float area = (v_3 - v_2).cross(v_1 - v_2).length() / 2;
    ...
    vertices.insert(vertices.end(), output_vertices.begin(), output_vertices.end());
}
  catch (const std::exception & e)
  {
