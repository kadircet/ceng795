...
class Mesh : public Shape {
 public:
  ...
  Mesh(int material_id, int texture_id, std::vector<Shape*>& triangles, const Transformation& b_transform, const Vector3& velocity)
      : material_id(material_id), texture_id(texture_id), bvh(NULL), base_transform(b_transform), velocity(velocity) {
  ...
  Vector3 velocity;
    ...
    if (velocity == zero_vector)
    {
      ...
      Ray ray_local(inverse_transformation.multiply(ray.o),
        inverse_transformation.multiply(ray.d, true), ray.ray_type, ray.time);
      ...
      if (mesh_->intersect(ray_local, hit_data))
      {
        hit_data.normal = transformation.get_normal_transformation_matrix().multiply(hit_data.normal, true).normalize();
        hit_data.shape = this;
        return true;
      }
      return false;
	...
	Mesh_instance(int material_id, int texture_id, const Mesh* mesh, const Transformation& transformation, const Vector3& velocity)
		: material_id(material_id), texture_id(texture_id), mesh_(mesh), transformation_(transformation), velocity(velocity),
