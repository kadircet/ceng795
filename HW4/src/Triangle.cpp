      ...
      scene_(scene)
{
  ...
  if (is_identity_) {
	  ...
	  bounding_box_ = Bounding_box::apply_transform(Bounding_box(min_c, max_c), transformation_);
  }
...
bool Triangle::intersect(const Ray& ray, Hit_data& hit_data) const {
  ...
  if (is_identity_) {
    ...
    const Ray ray_local(inverse_transformation.multiply(ray.o),
      ...
      hit_data.normal = normal_transformation.multiply(this->normal, true).normalize();
		  return true;
	  }
	  return false;
  }
