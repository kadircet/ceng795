...
Bounding_box Bounding_box::apply_transform(const Bounding_box& bounding_box, const Transformation& transform) {
	const Matrix4x4& transformation_matrix = transform.get_transformation_matrix();
	const Vector3& min = bounding_box.min_corner;
	const Vector3& max = bounding_box.max_corner;

	const Vector3 v000 = transformation_matrix.multiply(Vector3(min.x, min.y, min.z));
	const Vector3 v001 = transformation_matrix.multiply(Vector3(min.x, min.y, max.z));
	const Vector3 v010 = transformation_matrix.multiply(Vector3(min.x, max.y, min.z));
	const Vector3 v011 = transformation_matrix.multiply(Vector3(min.x, max.y, max.z));
	const Vector3 v100 = transformation_matrix.multiply(Vector3(max.x, min.y, min.z));
	const Vector3 v101 = transformation_matrix.multiply(Vector3(max.x, min.y, max.z));
	const Vector3 v110 = transformation_matrix.multiply(Vector3(max.x, max.y, min.z));
	const Vector3 v111 = transformation_matrix.multiply(Vector3(max.x, max.y, max.z));
