...
const std::vector<Vector3> Perlin_noise::g_ = {Vector3(1, 1, 0), Vector3(-1, 1, 0), Vector3(1, -1, 0), Vector3(-1, -1, 0),
...
Perlin_noise::Perlin_noise(const std::string& pn_appearance, float scaling_factor)
...
float Perlin_noise::get_value_at(const Vector3& position) const {
  ...
  for (int i = x; i <= x + 1; i++)
  {
    for (int j = y; j <= y + 1; j++)
    {
      for (int k = z; k <= z + 1; k++)
      {
        const Vector3 v(scaled_position.x - i, scaled_position.y - j, scaled_position.z - k);
